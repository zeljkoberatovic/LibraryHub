...
@Component({
  selector: 'app-mark-as-lost',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './mark-as-lost.component.html',
  styleUrls: ['./mark-as-lost.component.css']
})
export class MarkAsLostComponent implements OnInit {
  rentedCopies: any[] = [];
  bookId?: number;
  ...
  markAsLost() {
    ...
    if (selectedIds.length === 0) {
      alert('Niste odabrali nijednu knjigu za otpis.');
      return;
    }
    // rentalService.markAsLost(selectedIds).subscribe(...)
    alert('Otpis knjiga je spreman za backend!');
  }
